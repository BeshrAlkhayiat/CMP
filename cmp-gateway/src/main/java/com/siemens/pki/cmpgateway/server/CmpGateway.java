...
public final class CmpGateway {
    ...
    private final class RestUpstream implements UpstreamExchange, CmpRaComponent.GeneratedKeyProvider {
        ...
        public byte[] sendReceiveMessage(
            ...
            // NOTE: the request type is deliberately NOT set here. The RA component
            // already stores the real initial request type in the SAME PersistencyContext
            // instance it hands to this callback (RaDownstream.handleP10CertificateRequest
            // -> persistencyContext.setRequestType(...)). Overwriting it with the body
            // type of the current message would corrupt the enrollment state for every
            // follow-up message of the transaction: OpenSSL's CERTCONF (body type 23)
            // reached the RA as "requestType = 23", and after this callback returned the
            // CertConfirm response, CmpRaUpstream re-read getRequestType() as 23 - which
            // equals PKIBody.TYPE_P10_CERT_REQ (also 23). RaDownstream.processCertResponse
            // then demanded a CertRep for a "P10CR" and threw
            // "unexpected CMP message type at upstream: ConfirmConf", surfaced to the
            // client as systemFailure. Worse, the stored P10CR body was retried against
            // issuePkcs10(), hitting CertificationRequest.getInstance() on a
            // DEROctetString (ClassCastException).
            //
            // Also do not touch any other field of the shared context (no nonce or
            // transaction-state updates): the downstream TransactionStateTracker owns
            // that state machine, and updating it from here made it treat our own
            // responses as second requests ("transactionIdInUse ... got CertRep").
            ...
            try {
                switch (currentBodyType) {
                    ...
                    case PKIBody.TYPE_CERT_CONFIRM:
                        ...
                        // from the request header via
                        // PkiMessageGenerator.buildRespondingHeaderProvider().
                        // Two deliberate choices here, both learned the hard way:
                        //  * Do NOT re-protect the answer with MsgOutputProtector: the
                        //    RA's upstream InputValidator runs MessageBodyValidator on
                        //    every returned message and rejects any non-empty protected
                        //    ConfirmConf body ("broken protection of ConfirmConf").
                        //  * Do NOT echo the request's sender/recipient or reuse its
                        //    generalInfo: buildRespondingHeaderProvider swaps sender and
                        //    recipient and sets recipNonce = request's senderNonce,
                        //    which is what the downstream nonce chain expects. An
                    ...
                    case PKIBody.TYPE_CRL_ANN:        // CrlOutBoundNewest (5.3.28)
                        ...
                        // and CrlAnnounce responses to RevRep bodies.
                    ...
                    case PKIBody.TYPE_CONFIRM:
                        // PkiAcceptConf (5.3.15/5.3.29) arriving at the upstream
                        // interface - a stray or replayed confirmation of a previous
                        // CertConfirm. Same handling as CertConfirm: answer with an
                        // unprotected ConfirmConf. A protected RevRep is NOT an option
                        // here: the RA's upstream InputValidator only accepts response
                        // types that match the initial request type (P10CR -> CertRep),
                        // so a RevRep would be rejected with "unexpected CMP message
                        // type at upstream" and surface to the client as systemFailure.
                        LOG.info("acknowledging PkiAcceptConf (body type {}) with"
